...
def is_trend_up(code, date, ma_window):
    df = etf_price_map.get(code)
    if df is None: return False
    sub = df[df['date'] <= date]
    if len(sub) < ma_window: return False
    return float(sub['close'].iloc[-1]) > float(sub['close'].iloc[-ma_window:].mean())
...
print(f"v12c: 放宽版双动量 | 60日>{MOM_THRESHOLD*100:.0f}% + MA20 | Top{TOP_N} | {STOP_LOSS*100:.0f}%止损 | 月度")
print("=" * 80)
...
for i, date in enumerate(dates):
    ...
    # 月度调仓
    if is_first_trading(date) and i >= 100:
        tv = total_value(date)
        ...
        for code in sector_codes:
            ...
            if mom is not None and mom > MOM_THRESHOLD and is_trend_up(code, date, MA_TREND):
        ...
        if i < 300 or i % 500 == 0:
            ...
            print(f"{date.strftime('%Y-%m-%d')}: 总={tv:.0f}, 现金={cash:.0f}, 目标数={len(targets)}, {pos_detail}")
...
print(f"\n\n{'指标':<20} {'v12c 放宽':>15} {'v10c':>15} {'沪深300':>15}")
print("=" * 80)
...
results.to_csv(os.path.join(CACHE_DIR, '..', 'backtest_v12c.csv'), index=False)
