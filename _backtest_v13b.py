...
=====================================
在v13基础上增加:
  - 牛市 (510300 > MA60): 正常运作, 50%核心 + 50%卫星
  - 熊市 (510300 < MA60): 强制至少60%债基, 最多40%权益
    - 权益中: 50%核心510300 + 50%卫星动量
    - 即: 20%核心 + 20%卫星 + 60%债基
...
STOP_LOSS = 0.15
TXN_COST = 0.001
BULL_BEAR_MA = 60       # 牛熊判断: MA60
BULL_EQUITY_MAX = 1.00   # 牛市最大权益比例
BEAR_EQUITY_MAX = 0.40   # 熊市最大权益比例 (至少60%债基)
CORE_IN_EQUITY = 0.50    # 权益中核心占比
...
def check_bull_bear(date):
    """牛熊判断: 510300 vs MA60"""
    sub = hs300_df[hs300_df['date'] <= date]
    if len(sub) < BULL_BEAR_MA + 1:
        return 'bear'  # 数据不足默认熊市
    price = float(sub['close'].iloc[-1])
    ma60 = float(sub['close'].iloc[-BULL_BEAR_MA:].mean())
    return 'bull' if price > ma60 else 'bear'
...
print("v13b: v13 + 牛熊判断 | 510300>MA60=牛市(满仓) | <MA60=熊市(60%债基)")
print(f"    牛市: 50%核心+50%卫星 | 熊市: 20%核心+20%卫星+60%债基")
print(f"    Top{TOP_N} 60日动量(无过滤) | {STOP_LOSS*100:.0f}%止损 | 月度调仓")
print("=" * 80)
...
for i, date in enumerate(dates):
    ...
    if is_first_trading(date) and i >= 80:
        ...
        # 1. 牛熊判断
        market = check_bull_bear(date)
        if market == 'bull':
            equity_ratio = BULL_EQUITY_MAX   # 100% 权益
            core_ratio = 0.50                # 50% 核心
            sat_ratio = 0.50                 # 50% 卫星
        else:
            equity_ratio = BEAR_EQUITY_MAX   # 40% 权益
            core_ratio = 0.50                # 权益中50%核心 = 20%
            sat_ratio = 0.50                 # 权益中50%卫星 = 20%

        core_value = tv * equity_ratio * core_ratio
        sat_value = tv * equity_ratio * sat_ratio
        ...
        if i < 200 or i % 400 == 0:
            ...
            state = "牛市" if market == 'bull' else "熊市"
            print(f"{date.strftime('%Y-%m-%d')} [{state}] 权益{equity_ratio*100:.0f}%: 总={tv:.0f}, 现金={cash:.0f}, {pos_detail}")
...
print(f"\n\n{'='*85}")
print(f"{' v13b 回测报告 (v13 + 牛熊判断) ':=^85}")
print(f"{'='*85}")
print(f"{'指标':<20} {'v13b 牛熊':>15} {'v13 无择时':>15} {'v10c':>15} {'沪深300':>15}")
print("-" * 85)
print(f"{'总收益':<20} {total_ret*100:>14.2f}% {'69.23%':>15} {'28.98%':>15} {hs_total*100:>14.2f}%")
print(f"{'年化收益':<20} {ann_ret*100:>14.2f}% {'6.44%':>15} {'3.06%':>15} {hs_ann*100:>14.2f}%")
print(f"{'年化波动':<20} {ann_vol*100:>14.2f}% {'27.10%':>15} {'9.42%':>15} {hs_vol*100:>14.2f}%")
print(f"{'夏普比率':<20} {sharpe:>15.3f} {'0.238':>15} {'0.325':>15} {hs_sharpe:>15.3f}")
print(f"{'最大回撤':<20} {max_dd*100:>14.2f}% {'-48.06%':>15} {'-19.67%':>15} {hs_maxdd*100:>14.2f}%")
print(f"{'收益/回撤比':<20} {abs(total_ret/max_dd):>15.3f} {'1.441':>15} {'1.473':>15} {abs(hs_total/hs_maxdd):>15.3f}")
print("=" * 85)
...
results.to_csv(os.path.join(CACHE_DIR, '..', 'backtest_v13b.csv'), index=False)
