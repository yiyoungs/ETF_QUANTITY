"""v13e: MA120 牛熊判断 + v13架构
=====================================
- 核心: 50% 510300
- 卫星: 50% Top3 60日动量(无过滤)
- 牛熊判断: 510300 vs MA120
  - 牛市 (price > MA120): 100% 权益
  - 熊市 (price < MA120): 40% 权益, 60% 债基
- 止损: 15%
- 月度调仓
"""
import pandas as pd
...
def calc_equity_ratio(date):
    ...
    ma120 = float(sub['close'].iloc[-BULL_BEAR_MA:].mean())
    return BULL_EQUITY if price > ma120 else BEAR_EQUITY
...
print("v13e: MA120牛熊 + v13架构 | 牛市100%权益 | 熊市40%权益(60%债基)")
...
print(f"\n\n{'='*100}")
print(f"{' v13e 回测报告 (MA120牛熊 + v13架构) ':=^100}")
print(f"{'='*100}")
print(f"{'指标':<20} {'v13e(MA120)':>14} {'v13d(MA200)':>14} {'v13(无择时)':>14} {'v10c':>12} {'沪深300':>12}")
print("-" * 100)
print(f"{'总收益':<20} {total_ret*100:>13.2f}% {'45.43%':>14} {'69.23%':>14} {'28.98%':>12} {hs_total*100:>11.2f}%")
print(f"{'年化收益':<20} {ann_ret*100:>13.2f}% {'4.54%':>14} {'6.44%':>14} {'3.06%':>12} {hs_ann*100:>11.2f}%")
print(f"{'年化波动':<20} {ann_vol*100:>13.2f}% {'21.34%':>14} {'27.10%':>14} {'9.42%':>12} {hs_vol*100:>11.2f}%")
print(f"{'夏普比率':<20} {sharpe:>14.3f} {'0.213':>14} {'0.238':>14} {'0.325':>12} {hs_sharpe:>12.3f}")
print(f"{'最大回撤':<20} {max_dd*100:>13.2f}% {'-37.96%':>14} {'-48.06%':>14} {'-19.67%':>12} {hs_maxdd*100:>11.2f}%")
print(f"{'收益/回撤比':<20} {abs(total_ret/max_dd):>14.3f} {'1.197':>14} {'1.441':>14} {'1.473':>12} {abs(hs_total/hs_maxdd):>12.3f}")
print("=" * 100)
...
results.to_csv(os.path.join(CACHE_DIR, '..', 'backtest_v13e.csv'), index=False)
