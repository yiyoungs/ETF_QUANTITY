...
def generate_extreme_crash_data():
    """生成极端崩盘数据，确保止损被触发"""
    ...
    prices = np.ones(n_days) * 3.0
    prices[:100] = np.linspace(3.0, 4.0, 100)
    prices[100:120] = np.linspace(4.0, 3.68, 20)
    prices[120:200] = np.linspace(3.68, 4.2, 80)
    prices[200:230] = np.linspace(4.2, 3.0, 30)
    prices[230:] = np.linspace(3.0, 3.8, n_days - 230)
    
    prices += np.random.normal(0, 0.008, n_days)
...
def test_extreme_crash():
    ...
    print("\n" + "=" * 80)
    if all_same:
        print("  ⚠️ 警告：所有参数组合结果相同！止损可能没有被触发")
    else:
        print("  ✅ 参数敏感性测试正常，不同止损阈值产生不同结果")
    print("=" * 80)
